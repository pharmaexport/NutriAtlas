...
# Small, stable subset used by search results, cards and cumul calculations.
# The full CIQUAL fiche is exported separately in `fullNutrients` below.
NUTRIENT_MAP = {
    "energy_kcal": [
...
def label_from_source_column(source_column_name: str) -> str:
    ...
    parenthetical_chunks = re.findall(r"\(([^()]*)\)", label)
    if parenthetical_chunks:
        last_chunk = parenthetical_chunks[-1].lower().replace("μ", "µ")
        if "100" in last_chunk and "g" in last_chunk:
            label = re.sub(r"\s*\([^()]*\)\s*$", "", label)
    return re.sub(r"\s+", " ", label).strip()


def unit_from_source_column(source_column_name: str) -> str:
    parenthetical_chunks = re.findall(r"\(([^()]*)\)", source_column_name)
    for chunk in reversed(parenthetical_chunks):
...
def nutrient_label(nutrient: sqlite3.Row | None) -> str:
    source_column_name = str(row_value(nutrient, "source_column_name", "sourceColumnName", "name", "label") or "")
...
def build_index() -> dict[str, Any]:
    ...
    all_nutrients = {
        int(row["id"]): row
        for row in conn.execute("SELECT * FROM nutrients ORDER BY id ASC")
    }
