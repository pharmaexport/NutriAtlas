...
export function FoodDetailClient({ food, portions, nutrients }: Props) {
  ...
  return (
    <section className="foodPage pageSection">
      ...
      {highlights.length > 0 ? <section className="highlightCard"><span>Contributions principales</span><div>{highlights.map((item) => <strong className={progressTone(item.role, item.percent)} key={item.key}>{item.label} · {percentLabel(item.percent)}</strong>)}</div></section> : null}
      <section className="nutrientTable nutrientDashboard"><div className="tableHeader"><span>Valeurs pour la portion</span><span>% des repères profil</span></div>{rows.map((nutrient) => { const width = Math.min(nutrient.percent || 0, 100); const overflowWidth = Math.min(Math.max((nutrient.percent || 0) - 100, 0), 100); const overflow = overflowLabel(nutrient.percent, nutrient.role); return <div className={`nutrientLine nutrientProgress ${progressTone(nutrient.role, nutrient.percent)}`} key={nutrient.key}><div className="nutrientMain"><span>{nutrient.label}</span><strong>{nutrient.value} {nutrient.unit}</strong><small>{referenceText(nutrient.reference)}</small>{nutrient.reference?.note ? <small>{nutrient.reference.note}</small> : null}</div><div className="progressTrack"><i style={{ width: `${width}%` }} />{overflowWidth > 0 ? <b style={{ width: `${overflowWidth}%` }} /> : null}</div><em>{percentLabel(nutrient.percent)}{overflow ? <small>{overflow}</small> : null}</em></div>; })}</section>
